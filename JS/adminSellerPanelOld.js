...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function deleteProduct(productId) {
    ...
    newConfirmBtn.addEventListener("click", function () {
      ...
      // Immediately remove the product card from the category section if we're in that view
      const categoryGrid = document.getElementById("categoryProductGrid");
      if (categoryGrid) {
        // Find and remove the card with the deleted product ID
        ...
        // If no products left, show "No products found" message
      ...
      // Reload products for other views
      loadProducts();
...
document.getElementById("updateProduct").addEventListener("click", function () {
  ...
  if (!title || !category || !brand || !price || !stock || !about) {
    ...
    showBootstrapToast("Please fill in all required field.", "warning");
    return;
  }
  ...
  // Update product
  // In the update product section, preserve these fields:
  ...
  if (imageFiles.length > 0) {
    ...
    Promise.all(imagePromises).then((images) => {
      ...
      // Immediately update the product card in the category section if we're in that view
      updateProductCardInCategory(updatedProduct);

      // Force reload products for other views
      loadProducts();
    });
  } else {
    products[productIndex] = updatedProduct;
